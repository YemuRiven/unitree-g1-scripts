...
# 新增：事件与标志（用于“本帧写完再停” + 可视化统计）
toggle_evt = Event()        # 按 s 置位；主循环消费后清零
pending_stop = False        # 录制中收到 s → 本帧 add_item() 后保存并停止
frame_in_episode = 0        # 当前 episode 写入的帧数（用于日志/OSD）
...
if __name__ == '__main__':
    ...
    if WRIST and args.sim:
        ...
        img_client = ImageClient(tv_img_shape=tv_img_shape, tv_img_shm_name=tv_img_shm.name,
                                 wrist_img_shape=wrist_img_shape, wrist_img_shm_name=wrist_img_shm.name, server_address="192.168.8.30")
        ...
        img_client = ImageClient(tv_img_shape=tv_img_shape, tv_img_shm_name=tv_img_shm.name, server_address="192.168.8.30", port=5555)

    print("主控脚本 image_receive_thread 启动")
    ...
    try:
        ...
        while running:
            ...
            # ===== GUI 显示 / 键盘（非 headless） + OSD 标识 =====
            if not args.headless:
                tv_resized_image = cv2.resize(tv_img_array, (tv_img_shape[1] // 2, tv_img_shape[0] // 2))

                # 叠加录制状态 OSD
                overlay = tv_resized_image.copy()
                bar_h = 36
                ...
                if is_recording:
                    cv2.circle(tv_resized_image, (18, 18), 8, (0, 0, 255), -1)         # 红点
                    status = f"REC  frames={frame_in_episode}"
                elif pending_stop:
                    cv2.circle(tv_resized_image, (18, 18), 8, (0, 165, 255), -1)       # 橙点：本帧写完即停
                    status = "REC  stopping after this frame..."
                else:
                    cv2.circle(tv_resized_image, (18, 18), 8, (0, 255, 0), -1)         # 绿点：未录制
                    status = "REC OFF"
                ...
                if key == ord('q'):
                    ...
                    logger_mp.info("[GUI] s → toggle recording requested")
                    toggle_evt.set()
                elif key == ord('a'):
                    if args.sim:
                        publish_reset_category(2, reset_pose_publisher)
            ...
            # ===== 录制：打包并 add_item（仅 is_recording=True） =====
            if args.record:
                # 末端状态/动作
                ...
                if is_recording:
                    ...
                    # 如果本帧前已请求停止：本帧 add_item 完成后立即保存并停止
                    if pending_stop:
                        pending_stop = False
                        is_recording = False
                        try:
        ...
        # 兜底：若仍在录制，退出前保存一次
        try:
            if args.record and 'is_recording' in globals() and is_recording:
                logger_mp.warning("[REC] finally: recording active on exit → save_episode()")
                recorder.save_episode()
        except Exception:
            logger_mp.exception("[REC] finally: save_episode() failed")
